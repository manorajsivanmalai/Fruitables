...
const Card = ({products,setProducts,set}) => {
    ...
    useEffect(() => {
        setTotal(products.filter((item) => item.card === true).reduce((total, item) => total + (item.price * item.quantity), 0));
   set(  products.filter((item) => item.card === true).reduce((sum,item)=>{
