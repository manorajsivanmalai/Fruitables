        ...
        axios
          ...
          .then((response) => {
            const user = response.data.find(
              (user) =>
                user.username === usersCretendial.username &&
                user.password === usersCretendial.password
            );
            if (user) {
              setLoginSuccess(true);
      ...
      <div className="Sigin">
        <div className="sigin-sub">
          ...
          <div className="sigin-btn">
            <button
              onClick={() => setActive({ login: true, siginup: false })}
              style={{ backgroundColor: active.login ? "#81c408" : "blue" }}
            >
              login
            </button>
            <button
              onClick={() => setActive({ login: false, siginup: true })}
              style={{ backgroundColor: active.siginup ? "#81c408" : "blue" }}
            >
              siginup
            </button>
          </div>
          ...
          >
            ...
            <input
              ...
              onChange={(e) =>
                setUsers({ ...users, password1: e.target.value })
              }
              style={{ display: active.siginup ? "block" : "none" }}
            />
            <button type="submit">sigin Up</button>
          </form>
          <div className="skip">
            <button onClick={(e) => setLoginSuccess(true)} className="btn-skip">
              skip
            </button>
          </div>
        </div>
